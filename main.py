...
def add_weather(city: str, tree: ttk.Treeview, status_label: tk.Label):
    ...
    if not weather_data or "error" in weather_data:
        status_label.config(text=f"Ошибка: {weather_data.get('error', 'Неизвестная ошибка')}")
        return
    ...
    cursor.execute("SELECT city_id FROM cities WHERE city_name = ?", (weather_data["city"],))
    city_record = cursor.fetchone()
    
    if city_record:
        city_id = city_record[0]
    else:
        cursor.execute("INSERT INTO cities (city_name) VALUES (?)", (weather_data["city"],))
    ...
    if weather_record:
        cursor.execute("""
            ...
            WHERE city_id = ?
        """, (weather_data["temperature"], weather_data["description"], timestamp, city_id))
    else:
        cursor.execute("""
            INSERT INTO weather (city_id, temperature, description, timestamp)
            VALUES (?, ?, ?, ?)
        """, (city_id, weather_data["temperature"], weather_data["description"], timestamp))
...
def create_gui():
    ...
    tk.Button(btn_frame, text="Добавить/Обновить", 
              command=lambda: add_weather(city_entry.get(), tree, status_label)).pack(side=tk.LEFT, padx=5)
    tk.Button(btn_frame, text="Поиск", 
              command=lambda: search_weather(city_entry.get(), tree, status_label)).pack(side=tk.LEFT, padx=5)
    tk.Button(btn_frame, text="Удалить", 
              command=lambda: delete_weather(city_entry.get(), tree, status_label)).pack(side=tk.LEFT, padx=5)
    tk.Button(btn_frame, text="Обновить таблицу", 
              command=lambda: refresh_tree(tree)).pack(side=tk.LEFT, padx=5)
